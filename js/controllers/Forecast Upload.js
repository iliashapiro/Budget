app.controller('ForecastUploadCtrl', ['$scope', '$rootScope', '$log', '$tm1Ui', '$timeout',  function($scope, $rootScope, $log, $tm1Ui, $timeout) {
   /*
    ...
    */
    ...
    $scope.read = function (workbook) {
        ...
        $timeout(function(){
            $scope.departmentRecordArray = [];
             $scope.departmentValidated = false;
             $scope.accountValidated = false;
              $scope.projectValidated = false;
    ...
    $scope.parseWorksheet = function(sht){
       ...
       $scope.lists.validation = [];
        for(var ww = 0; ww < $scope.lists.records.length; ww++){
            $scope.lists.validation.push($scope.lists.records[ww]);
        }
...
//            $scope.lists.allowedPeriods = [];
    ...
    $scope.validate = function() {
        ...
        if ($scope.lists.validation.length > 0){
            ...
            var count = 0;
            $scope.batchArrayCount++;
            // Check for duplicate record
            for(var vv = 0; vv < $scope.lists.records.length; vv++){
                if(record.DepartmentProjectAccount == $scope.lists.records[vv].DepartmentProjectAccount){
                    count++;
                }
            }
            if(count > 1){
            ...
            if(isNaN(record.m01) ||isNaN(record.m02) ){
             ...
             for(var nn= 0 ; nn < $scope.lists.tm1DepartmentList.length; nn++){
                var obj = $scope.lists.tm1DepartmentList[nn];
                    //  console.log(obj.key, record.Department, "Validated department");
                if((record.Department+"") === (obj.key+"")){
                    // record.validated = true;
                    $scope.departmentValidated = true;
                        record.selected = false;
                    //  console.log("Validated Department", record.Department);
                ...
                for(var nn= 0 ; nn < $scope.lists.tm1AccoutList.length; nn++){
                    var obj = $scope.lists.tm1AccoutList[nn];
                    // console.log('obj', obj.key);
                    if(record.AccountCode === obj.key){
                     //   console.log("Validated Account", record.AccountCode);
                        record.selected = false;
                        $scope.accountValidated = true;
                        
                    } 
                }
                //console.log($scope.accountValidated , $scope.departmentValidated, "$$$$$$$$" )
    ...
    $scope.save = function(){
        ...
        for(var rr = 0; rr < $scope.lists.records.length; rr++){
            var record = $scope.lists.records[rr];
            if (record.selected){
    ...
    $scope.saveItem = function(record){
        ...
        for(var ff = 0; ff < 12; ff++){
            if($scope.lists.allowedPeriodObj[$scope.lists.recordHeaders[ff].period] == "N"){
                var period = ff < 10 ? '0' + (ff + 1) : (ff + 1);
            cellPutRequests.push({
                value: record['m' + period],
                instance:"UofA_TM1_FP-Dev",
                cube: $scope.selections.cube,
                cubeElements:[$scope.selections.scenario,
                            $scope.lists.recordHeaders[ff].period,
                            record.Department,
                            record.AccountCode,
                            record.ProjectID,
