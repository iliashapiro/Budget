app.controller('ServicesCtrl', ['$scope', '$rootScope', '$timeout', '$state', '$stateParams', '$tm1Ui', '$log',
                                function($scope, $rootScope, $timeout, $state, $stateParams, $tm1Ui, $log) {
		...
		$tm1Ui.applicationAdminLogin($scope.values.user, $scope.values.pass).then(function(result){
			...
			if(result.success){
				...
				$tm1Ui.applicationAdminLogout().then(function(resultLogout){
					...
					test.result = resultLogout.message;			
	...
	$scope.testServices = function(){
		...
		$tm1Ui.applicationUserMenus().then(function(result7){
			if(angular.isArray(result7) && result7.length > 0){
				test7.success = true;
			}			
