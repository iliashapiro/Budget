...
//            $scope.lists.allowedPeriods = [];
    ...
    $scope.validate = function() {
        ...
        if ($scope.lists.validation.length > 0){
            ...
            $tm1Ui.dimensionElement("UofA_TM1_FP-Dev", "FP Current Department", record.Department).then(function(value){
                if (value == undefined){
                    record.selected = false;
                    record.errorMsg = record.errorMsg + record.Department + " is invalid.";
                }else{
                    
                    $tm1Ui.attributeGet("UofA_TM1_FP-Dev", "FP Current Department", record.Department,"Department Status").then(function(value){
                    ...
                    $tm1Ui.cellGet("UofA_TM1_FP-Dev", "FP Setting", "Current Planning Year - Forecast", "String").then(function(PlanningYear){
                        $tm1Ui.cellGet("UofA_TM1_FP-Dev", "FP Forecast", "All Projects", $scope.selections.scenario, PlanningYear.Value, record.Department,"NET OPERATING RESULT L1","Project Budget Flag").then(function(value){
                            if(value.Value != "Y"){
                            record.selected = false;
            ...
            $tm1Ui.dimensionElement("UofA_TM1_FP-Dev", "UPP Account", record.AccountCode).then(function(value){
                if (value == undefined){
                    record.selected = false;
                    record.errorMsg = record.errorMsg + record.AccountCode + " is invalid.";
                }else{
                    
                    $tm1Ui.attributeGet("UofA_TM1_FP-Dev", "UPP Account", record.AccountCode,"Account Status").then(function(value){
                        if (value.Value != "Active"){
                            record.selected = false;
